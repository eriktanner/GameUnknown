...
public class CallThruLocalPlayer : NetworkBehaviour
{
    ...
    void Start()
    {
        //I thought it would be a good idea to check if this component was attach to the local player and disable it if it wasn't
        //This broke the game because the spells are not actually created by the local player but by the server(for now)
